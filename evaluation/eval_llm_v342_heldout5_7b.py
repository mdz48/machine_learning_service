# v3.4.1: Regla 4 ampliada (no patea = sintoma presente) + fixes anteriores.
import json, re, sys, time, urllib.request
from eval_data_heldout5 import DATA
...
ALARM_SET = {"CEFALEA","VISION_BORROSA","EDEMA","DOLOR_EPIGASTRICO","SANGRADO",
...
OUTF="/tmp/llm7b_heldout5_v342.jsonl"; open(OUTF,"w").close()
call("calentando")
for i,c in enumerate(DATA):
    try: syms,dt=call(c["text"])
    except Exception: syms,dt=[],-1
    kept=[s for s in syms if s.get("code") in ALARM_SET or s.get("code")=="OTRO" or grounded(s.get("raw_text",""), c["text"])]
    dropped=[s for s in syms if s.get("code") not in ALARM_SET and s.get("code")!="OTRO" and not grounded(s.get("raw_text",""), c["text"])]
...
open("/tmp/llm7b_heldout5_v342.done","w").write("ok")
